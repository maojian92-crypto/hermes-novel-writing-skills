---
name: core-constraints
purpose: 每次章节写作前加载，确保三大定律执行
---

# 核心约束文档

> 主服务: 章节写作 Step 2
> 次服务: 审查 Step 4
> 内容层级: 缺陷补偿层（网文特定防幻觉协议）

---

## 三大定律（低自由度 - 必须精确执行）

| 定律 | 规则 | 检查方式 |
|------|------|----------|
| **大纲即法律** | 严格执行大纲，不得擅自发挥 | 审查时对照大纲 |
| **设定即物理** | 实力/招式/物品 ≤ 已有记录 | 写作前查询 Truth Files 确认 |
| **发明需识别** | 新实体必须记录，不得凭空创造 | 完成后更新 Truth Files |

---

## 章节约束分层

### Hard（必须）

- 本章可读性达标：读者能回答"发生了什么 / 谁在做什么 / 为什么"。
- 本章必须存在清晰推进：问题、目标、代价、关系变化、信息变化至少一项可被识别。
- 若上章有明确承诺（钩子/未闭合问题），本章必须回应（允许部分兑现，不要求一次性结清）。
- 禁止输出占位正文（如 `[待补充]`、`[TODO]`、`...（省略）...`）。
- 叙述人称：全书第三人称，严禁"我"叙述（对话中除外）。

### Soft（建议）

- 开头尽早进入冲突/风险/强情绪（建议前 200-400 字）。
- 未闭合问题或下一章期待锚点建议放在章末或后段。
- 局面变化建议保持节奏感（参考 800-1400 字一个脉冲）。
- 微兑现频率：每章至少1个"小收获"让读者感觉"这章没白看"。

### Style（可选强化）

- 对话尽量带意图（试探/回避/施压/诱导），减少纯说明句。
- 避免连续大段纯解释；若必须解释，建议切分为"信息 + 行动/反应"。
- 避免"回去休息了"式机械收尾；若使用平缓收尾，需同时保留未闭合期待。

---

## Anti-AI 写作规范

- 禁止连续 3 段以上使用相同句式结构。
- 情绪描写必须通过行为/生理暗示，禁止直接标签（"他感到X"）。
- 每 500 字至少有一次节奏变化（短句爆发、对话插入、场景切换）。
- 长段落（800-1400字）建议包含一个节奏脉冲（信息密度变化或情绪转折）。
- 对话必须带意图冲突，禁止纯信息传递。
- 章末禁止"安全着陆"——必须保留至少一个未解决的问题或不安感。
- 删掉万能副词（缓缓/淡淡/微微），用具体动作替代。

---

## 爽点与节奏

- 爽点密度由题材与章型共同决定：过渡章允许低密度，但不允许"整章无收获"。
- 组合爽点与里程碑爽点采用滚动窗口评估（5章/10-15章），用于预警而非逐章硬判。
- 连续同类型爽点达到 3 章记为风险预警，优先通过类型变体或执行差异化修正。

---

## 三线节奏红线

| 情节线 | 警告条件 |
|--------|----------|
| Quest（主线） | 连续 5+ 章 |
| Fire（感情线） | >10 章未出现 |
| Constellation（世界观） | >15 章未出现 |

---

## 禁止事项

- `[待补充]`、`[TODO]`、`...（省略）...` → 必须完整写出
- 战斗后无善后描述
- 新实体无记录直接登场
- 角色能力超越当前设定无解释

---

## 示例

**示例1：新技能使用**
```
输入: 主角需要使用"天雷掌"击败敌人

处理:
1. 查询 Truth Files 中是否有"天雷掌"技能
2. 若有：直接使用
3. 若无：在正文中描写获得途径（如拜师/领悟/传承），完成后更新 Truth Files
```

**示例2：境界突破**
```
输入: 剧情需要主角展示筑基期实力，但 Truth Files 显示练气期

处理:
❌ 直接写筑基期战力 → 违反"设定即物理"
✅ 先安排突破场景，更新 Truth Files，再展示新实力
```

---

## 错误处理

| 错误 | 正确处理 |
|------|----------|
| 新实体描写模糊 | 确保新实体有明确名称和描写 |
| 主角突然会新技能 | 先描写获得途径 |
| 实力设定不一致 | 写作前查询 Truth Files 确认 |
| 整章无推进点 | 补至少一项可识别推进 |
| 占位符未替换 | 必须完整写出所有内容 |
